...
class LocalTab:
    ...
    def _setup_ui(self):
        ...
        file_frame.grid(row=0, column=0, pady=(5, 10))
        file_frame.grid_columnconfigure(1, weight=1)

        file_label = ctk.CTkLabel(file_frame, text="Video File:", text_color="#3a7ebf", 
                                font=ctk.CTkFont(family="Microsoft Sans Serif", weight="bold", size=15))
        file_label.grid(row=0, column=0, pady=(10, 5))
        ...
        self.local_file_display.grid(row=0, column=1, padx=(10, 10), pady=(10, 5))
        ...
        self.local_file_button.grid(row=0, column=2, pady=(10, 5))
        ...
        lang_frame.grid(row=0, column=0, padx=5, pady=5, columnspan=2)
        ...
        subtitles_frame.grid(row=1, column=0, padx=10, pady=5, columnspan=2)
        ...
        self.status_frame.grid(row=5, column=0, padx=10, pady=(5, 10))
        ...
        log_frame.grid(row=6, column=0, padx=10, pady=(10, 5))
    ...
    def toggle_subtitles_option(self):
        self.app.local_add_subtitles = self.subtitles_checkbox.get()
        self.app.translator.log_with_emoji(f"Subtitles {'enabled' if self.app.local_add_subtitles else 'disabled'} for local videos", emoji_type='SUBTITLES')
    ...
    def choose_local_file(self):
        ...
        if file_path:
            ...
            self.app.translator.log_with_emoji(f"Selected local file: {file_path}", emoji_type='FILE')

    def choose_output_dir(self):
        output_dir = filedialog.askdirectory()
        if output_dir:
            self.output_dir_display.configure(text=output_dir)
            self.app.translator.log_with_emoji(f"Local output directory set to: {output_dir}", emoji_type='FILE')
    ...
    def start_process(self):
        ...
        import threading
        threading.Thread(
            target=self.app.run_local_process,
            args=(file_path, from_lang, to_lang, output_dir, self.app.update_progress),
            daemon=True
        ).start()